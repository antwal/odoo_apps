...
class wizard_spesometro_export(osv.osv_memory):
    ...
    def _record_B(self, cr, uid, comunicazione, context=None):
        ...
        if not comunicazione.soggetto_codice_carica:
            raise osv.except_osv(_('Errore comunicazione!'),\
                _("Manca il Codice Carica del Soggetto "))
        ...
        rcd += '{:2s}'.format(str(comunicazione.soggetto_codice_carica).zfill(2)) # codice carica 
    ...
    def _record_C_BL(self, cr, uid, line, prog_modulo, prog_sezione, context=None):
        ...
        if not line.pg_denominazione and not line.pf_codice_stato_estero:
                raise osv.except_osv(_("Errore quadro BL"), _(" - Partner %s! Inserire  \
                    Codice Stato Estero per la persona fisica") % (line.partner_id.name ,) )
        # BL001009 : Codice stato estero x persona giuridica
        if line.pg_denominazione and not line.pg_codice_stato_estero:
                raise osv.except_osv(_("Errore quadro BL"), _(" - Partner %s! Inserire  \
                    Codice Stato Estero per la persona giuridica") % (line.partner_id.name ,) )
